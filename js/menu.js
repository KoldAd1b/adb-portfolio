...
const MENU_ITEMS = [
  ...
  { label: "Writing", icon: "newspaper-sharp", href: "/blog" },
  { label: "Esoterica", icon: "book-sharp", href: "/esoterica" },
  { label: "Academia", icon: "school-sharp", href: "/academia" },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  MENU_ITEMS.forEach((item, index) => {
    ...
    segment.addEventListener(
      "click",
      (e) => {
        if (isSamePage(segment.href) && !hasHashTarget(segment.href)) {
          e.preventDefault();
          e.stopPropagation();
...
function hasHashTarget(href) {
  return Boolean(getLinkUrl(href)?.hash);
}
